...
Here’s a breakdown of how it works in last example:
...
- Finally: This code runs no matter what—whether there’s an error, a warning, or success. It’s useful for logging, cleanup, or confirming that the operation finished.
...
### `r emoji::emoji("point_right")` Export only when asked and inform the user about it
...
The `.Renviron` approach works well for local development, but once it’s time to share the project, it’s time to level up your config game.
...
- The code just loads the correct configuration automatically, based on the current environment. 
...
<figure>
  <a href="https://rstudio.github.io/config/index.html" target="_blank">
    <img src="https://rstudio.github.io/config/logo.svg" alt="https://rstudio.github.io/config/" width="55%" align="center"/>
  </a>
</figure>
<br>
...
<div class="info-box">
  <i>`r emoji::emoji("parrot")`</i> All settings will be pulled from the appropriate section — no need to manually double-check; no need to rewrite part of your code. Just switch the environment variable like you switch a light switch, and your code will adapt automatically.
</div>
...
An example `config.yml` file is displayed on the right
